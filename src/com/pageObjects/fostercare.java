package com.pageObjects;

import org.openqa.selenium.By;

public interface fostercare {
	
	public static By curamUAIframe = By.xpath(".//*[@id='curamUAIframe']");
	
	public static By nextbtn = By.xpath("//span[contains(text(),'Next')]");
	
	
	public static By InFosterCare = By.id("__o3id6");
	
	public static By WhichState = By.id("__o3id8");
	
	public static By HowOld = By.id("__o3id9");
	
	
	public static By student = By.id("__o3id10");
	
	public static By studenttype = By.id("__o3id11");
	
	public static By schooltype = By.id("__o3id12");
	
	public static By EndDate = By.id("__o3id13");
		
	public static By enrollment = By.id("__o3ida");

}
