...
import com.utilities.LoggerClass;
		...
		public class MotherVtChild extends BaseClass {
			...
			public String AData[][],IData[][],GData[][],strt,citi,ste,zpcd;
			...
			public String incmtyp1,amt1,freq1,stdate1,newmem,newmem1,enrlmnt,enrlt1,empname,ctzn,incmtyp2,amt2,freq2,stdate2,Lname1;
...
//		 			d.findElement(By.xpath("//button[contains(@title,'Close subscription dialog')]")).click();
			...
			@Test
			public void TC1018() throws Exception{
				...
				//moreinfo_abtu.fillin(ssn,enrlmnt);  //--> Regular American
				moreinfo_abtu.fillin1(ssn,enrlmnt);  //--> American Indian 
				
				midWait(3);
						
				common_elements.click();  //--> Pregnant Question
				...
				//foster_care.fillin1();  // Foster Care - Change Applicant age between 18 & 25 Years 
				//midWait(9);   //uncoment this for foster care and comment next midwait(4)
				...
				//income_info.income(amt1);
				midWait(3);
				income_details.fillin1(incmtyp1,empname,strt,citi,ste,zpcd,amt1,freq1,stdate1);  //W2 with Employer details
			//	income_details.fillin1(incmtyp1,amt1,freq1,stdate1); // -> Non W2
				...
				common_elements.click();  // More about applicaton group
			
				common_elements.click();  // Additional information (Incarcerated)
				
				common_elements.click();  // Tribal Health Program (Enable when Applicant is American Indian)
