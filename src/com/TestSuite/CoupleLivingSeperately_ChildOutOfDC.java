...
public class CoupleLivingSeperately_ChildOutOfDC extends BaseClass {
	...
	public String AData[][],IData[][],GData[][],adr1,cti1,st1,zip1;
...
// 			d.findElement(By.xpath("//button[contains(@title,'Close subscription dialog')]")).click();
	...
	@Test 
	public void TC1005() throws Exception 
	{
		LoadVariables();
		logger.info("                  ");
	    ...
	    midWait(3);
		groupmembers_app.diffadd(f1name,Lname1,db1,gndr1,adr1,cti1,st1,zip1); // --> Original
	    groupmembers_app.fillin1(f1name,Lname1,db1,gndr1);  // --> Modified
	    midWait(3);
	    ...
	    midWait(3);
	//	groupmembers_app.diffadd1(f2name,Lname2,db2,gndr2,adr2,cti2,st2,zip2);  // --> Original
		groupmembers_app.fillin2(f2name,Lname2,db2,gndr2);  // --> Modified
	    midWait(3);
	...
	public void LoadVariables() throws Exception
	  {	 
		...
		if(GData[Grw][6].equalsIgnoreCase("Random")) {
	         ...
	         db2 = BusinessMethods.getDOB(Integer.parseInt(GData[Grw][5]));
		} else {
			ssn2 = GData[Grw+1][6];
		} // end if
		if(GData[Grw][6].equalsIgnoreCase("Random")) {
	          ssn3 = BusinessMethods.GetSSN();
	          db3 = BusinessMethods.getDOB(Integer.parseInt(GData[Grw+1][5]));
		} else {
			  ssn3 = GData[Grw+2][6];
		} // end if

		pswd=AData[Arw][3];sex=AData[Arw][9];addrl1=AData[Arw][11];st=AData[Arw][14];ct=AData[Arw][13];
		zip=AData[Arw][15];elig=AData[Arw][45];newmem=AData[Arw][56];enrlmnt=AData[Arw][50];//Lname=AData[Arw][6];dob =AData[Arw][7];
		...
		incmtyp3=IData[Irw+2][4];amt3=IData[Irw+2][12];freq3=IData[Irw+2][13];stdate3=IData[Irw+2][14];
