...
public class FourHouseHold extends BaseClass {
	...
	@Test 
	public void TC1031() throws Exception { 
		...
		FourthMemberDetails();  // **************Fourth Member ***************************		
		midWait(3);
	...
	public String[][] fetchdata(String Sheet, int ApplRows, int ApplCols) throws Exception
	 ...
	 public void FirstMemberDetails() throws Exception {
				...
				//   Primary MEC Enable  --> if you want MEC for Primary -> Enter 'Yes' in DataSheet (ARW-50)
...
/////////////////////////***End of 3rd Member********************************************************************	 
	 ...
	 public void FourthMemberDetails() throws Exception{
		 
		// 4th Group Member
		 String f3name,Lname3,ssn3,db3,gndr3;
		 ...
		 if(GData[Grw+2][2].equalsIgnoreCase("Random")) {
			...
			// db3 = GData[Grw+2][5];   // ------------------> Use this when Age < 1 Year
			 db3 = BusinessMethods.getDOB(Integer.parseInt(GData[Grw+2][5]));
			 gndr3=GData[Grw+2][7];
					 
		 } else {
			f3name= GData[Grw+1][2];
			 Lname3= GData[Grw+1][2];
			 ssn3 = GData[Grw+1][6];
			 db3 = GData[Grw+1][6];
			 gndr3=GData[Grw+1][7];
				...
				//   4th Member DC Resident OR Not

		 			//Non DC Resident
		 			/*String adr3,cti3,st3,zip3;
				 	adr2=GData[Grw+2][11];cti2=GData[Grw+2][13];st2=GData[Grw+2][14];zip2=GData[Grw+2][15];
					groupmembers_app.diffadd(f3name,Lname3,db3,gndr3,adr3,cti3,st3,zip3); // For different address <90Days*/
		 
					// DC Resident			
					groupmembers_app.fillin(f3name,Lname3,db3,gndr3); // For DC address
					midWait(5);
					
					 	
						
					// **  4th Member American Indian Or NOT 
					// **  Also update below function for Mother Enrolled in Medicaid when Age < 1 Yr
					
						String enrlt3=GData[Grw+2][44];

						// Indian
						//groupxtradetails_app.Indian(ssn3,enrlt3);    // Indian - also handle extra screen for tribal question at the end of application
						
						// Not Am Indian
						groupxtradetails_app.fillin(ssn3,enrlt3);  // Remove extra "next" line enabled for Am Indian
	...
	//*******************Any one of the following (Pregnant, Foster Care or Student) *********************
					...
					// Change the gender to Female,  Age >=17 years and give pregnant info in Data sheet (Grow 54 & 55)
						...
						/*	String PregKids,PregDueDt;
						PregKids=GData[Grw+2][54];PregDueDt=GData[Grw+2][55];		
						pregnant.fillin(PregKids,PregDueDt);		  				
						midWait(3);		*/		
						
						
						
					// **4th Member Foster Care				
					//	Change Age of Applicant between 18 & 25 Years 
     ...
     //*******************End of Pregnant, Foster Care or Student *********************	
					...
					// **4th Member MEC Details - enable if RW-Col-44 in Excelsheet is marked as "Yes"
					
					/*	String plan,date,benefits;
						plan=GData[Grw+2][45];date=GData[Grw+2][46];benefits=GData[Grw+2][49];  
						additional_info.fillin(plan,date,benefits);   			
						midWait(3);*/
...
/////////////////////////***********************************************************************
	public void FilingTaxes() throws Exception {
	 ...
	 //1.  4HH Wife, Husband,2 dependent children claimed by parents filing jointly
		
			tax_filer.click1();    // Selects first & second applicants only
			midWait(3);
			taxfiler_status.yes_filetaxes(); // filing jointly
			midWait(3);
			dependency_taxreturn.select1();  //First child claimed by 1st applicant
			midWait(3);
			dependency_taxreturn.select2();  // Second child claimed by 2nd Applicant 			
		
		
		
	 //2.  4HH Wife, Husband,1 dependent child and 1 Sibling who is not claimed by any one
		
			/*tax_filer.click2();  // selects firt, second and fourth applicants
			midWait(3);
			
			taxfiler_status.yes_filetaxes(); // Filling Jointly
			//taxfiler_status.no_filetaxes(); // Not filing jointly
			midWait(3);
			dependency_taxreturn.select1();
			
			midWait(3);
			dependency_taxreturn.select2NOs(); // Not claimed by this family or outside */
